...
// novi način spremanja slika, u buffer avatar: {type: Buffer}
...
const userSchema = new mongoose.Schema({
    ...
    avatar: {
        type: Buffer  // sprema sliku u Buffer MongoDB baze a ne u direktorij /images
    }  
...
userSchema.methods.toJSON = function() {
    ...
    delete userObject.password; // mičemo password i tokene iz objecta koji šaljeno na front end
    delete userObject.tokens;
    delete userObject.avatar;  // ne šaljemo avatar image jer opterećuje JSON slanje a i postoji poseban APi za to
...
userSchema.methods.generateAuthToken = async function () {
    ...
    //const token = jwt.sign({_id: user._id.toString()}, 'thisismynewcourse');  // stari način
    const token = jwt.sign({_id: user._id.toString()}, process.env.JWT_SECRET);  // novi način sa process.env
